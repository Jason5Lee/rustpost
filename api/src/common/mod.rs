...
impl UserName {
    ...
    pub fn try_new(input: String) -> Result<UserName, Invalid<String>> {
        if input.is_empty() {
            ...
            .chars()
            .all(UserName::is_legal_character)
        {
...
impl Password {
    pub fn to_encrypted(&self, cost: u32) -> String {
        bcrypt::hash(&self.plain, cost).expect("unable to hash")
    }

    pub fn verify_encrypted(&self, encrypted: &str) -> bool {
        bcrypt::verify(&self.plain, encrypted).expect("unable to query")
    }
